...
class PayrollDeduction
{
    ...
    {
        if (!$shopId) {
            return $this->db->fetchAll("SELECT * FROM payroll_deductions WHERE shop_id IS NULL ORDER BY code ASC");
        }
        return $this->db->fetchAll(
            "SELECT a.* FROM payroll_deductions a
             WHERE a.shop_id = ? OR (a.shop_id IS NULL AND NOT EXISTS (
                 SELECT 1 FROM payroll_deductions b WHERE b.code = a.code AND b.shop_id = ?
             ))
             ORDER BY a.code ASC",
            [$shopId, $shopId]
        );
    ...
    {
        return $this->db->fetch("SELECT * FROM payroll_deductions WHERE id = ?", [$id]);
    }

    public function findByCode($code, $shopId = null)
    {
        if ($shopId) {
            $sql = "SELECT * FROM payroll_deductions
                    WHERE code = ? AND (shop_id = ? OR shop_id IS NULL)
                    ORDER BY shop_id IS NULL ASC
                    LIMIT 1";
            return $this->db->fetch($sql, [$code, $shopId]);
        }
        return $this->db->fetch("SELECT * FROM payroll_deductions WHERE code = ? AND shop_id IS NULL LIMIT 1", [$code]);
    }
    ...
    public function update($id, $data)
    {
        ...
        $sql = "UPDATE payroll_deductions SET " . implode(", ", $fields) . " WHERE id = :id";
        return $this->db->execute($sql, $params);
    }

    public function delete($id)
    {
        return $this->db->execute("DELETE FROM payroll_deductions WHERE id = ?", [$id]);
    }
}
