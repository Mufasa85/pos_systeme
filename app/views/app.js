...
// ============================================
// ROUTER - SPA Loading System (Template-based)
// ============================================
const Router = {
  currentPage: 'dashboard',
  isLoading: false,

  loadComponent(templateName, containerId) {
    const container = document.getElementById(containerId);
    if (container && Templates[templateName]) {
  ...
  loadPage(pageName) {
    ...
    if (container && Templates[pageName]) {
      container.innerHTML = Templates[pageName];
...
function navigateTo(page) {
  ...
  $('#sidebar-container').classList.remove('open');
...
async function loadStaticComponents() {
  Router.loadComponent('header', 'header-container');
  Router.loadComponent('sidebar', 'sidebar-container');
  Router.loadComponent('login', 'login-container');
}

async function showLogin() {
  // Load login template if not already loaded
...
function updateUserInfo() {
  ...
  const mobileUser = $('#mobile-user-info');

  if (userName) userName.textContent = user.fullName;
  if (userRole) userRole.textContent = user.role === 'admin' ? 'Administrateur' : 'Vendeur';
  if (userAvatar) userAvatar.textContent = user.fullName.charAt(0).toUpperCase();
  if (mobileUser) mobileUser.textContent = user.fullName;
}
...
function initGlobalEventListeners() {
  ...
  if (closeReceipt) {
    closeReceipt.addEventListener('click', () => closeModal('receipt-modal'));
  }
  const validateReceiptBtn = $('#validate-receipt');
  if (validateReceiptBtn) {
    validateReceiptBtn.addEventListener('click', validateReceipt);
  }
...
function rebindPageEvents() {
  ...
  if (loginForm) {
    loginForm.addEventListener('submit', async (e) => {
      ...
      if (login(username, password)) {
        ...
        const loginError = $('#login-error');
        if (loginError) loginError.textContent = 'Identifiants incorrects ou compte desactive';
      }
    });
  }
  ...
  const validateSaleBtn = $('#validate-sale');
  if (validateSaleBtn) {
  ...
  if (productsFilter) {
    productsFilter.addEventListener('input', renderProductsTable);
  }
  ...
  if (dateFilter) {
    dateFilter.addEventListener('change', renderHistoryTable);
  }
  const sellerFilter = $('#seller-filter');
  if (sellerFilter) {
    sellerFilter.addEventListener('change', renderHistoryTable);
  }
  ...
  if (categoryForm) {
    categoryForm.addEventListener('submit', saveCategoryFn);
  }
  ...
  if (categoryColor) {
    categoryColor.addEventListener('input', (e) => {
      const colorHex = $('#category-color-hex');
...
// Global navigateTo for backward compatibility
function navigateTo(page) {
