      ...
      <div id="page-analytics" class="page <?= $page == 'analytics' ? 'active' : '' ?>">
        <div class="page-header">
          ...
          <div class="analytics-actions">
            ...
            <button class="btn btn-secondary btn-small" onclick="window.open(APP_URL+'/api/export/ventes?from=<?= date('Y-m-01') ?>&to=<?= date('Y-m-d') ?>','_blank')" title="Exporter CSV">
        ...
        <?php if (isset($_SESSION['role']) && $_SESSION['role'] === 'super_admin' && !empty($shops)): ?>
        <div style="margin-bottom:1rem">
          <select id="analytics-shop-filter" onchange="window.location.href='/analytics?shop_id='+this.value" style="padding:.4rem .8rem;border-radius:var(--radius,8px);border:1px solid var(--border,#e2e8f0)">
            <option value="">Toutes les boutiques</option>
            <?php foreach ($shops as $sh): ?>
              <option value="<?= $sh['id'] ?>" <?= (isset($_GET['shop_id']) && $_GET['shop_id'] == $sh['id']) ? 'selected' : '' ?>><?= htmlspecialchars($sh['nom']) ?></option>
            <?php endforeach; ?>
          </select>
        </div>
        <?php endif; ?>
