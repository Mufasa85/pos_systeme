...
class PayrollPaymentController extends Controller
{
    public function forPayslip($params)
    {
        ...
        if (!$payslipId) { $this->status(400)->json(['error' => 'ID payslip manquant']); return; }
    ...
    public function create()
    {
        ...
        if (!$id) { $this->status(500)->json(['error' => 'Erreur création']); return; }
    ...
    public function update($params)
    {
        ...
        if (!$payment) { $this->status(404)->json(['error' => 'Paiement introuvable']); return; }
    ...
    private function closePeriodIfAllPaid($periodId)
    {
        ...
        foreach ($payslips as $p) {
            if ($p['status'] !== 'paid') { $allPaid = false; break; }
        }
