...
const posCart = {
    ...
    calculateItemTTC(item, quantityOverride = null) {
        ...
        const lineHT = price * quantity + specificTaxUnit + vatOnSpecificTaxUnit;
    ...
    getPaymentInfoHtml(dgiResponse = null) {
        ...
        const totalTTC = this.currentTotals.total;

        // Build payments HTML: use stored payments if available, otherwise fall back to legacy type
        let paymentsHtml = '';
...
function getTaxRateFromGroup(group) {
    ...
    const cat = PROFORMA_TAX_CATEGORIES.find(c => c.label === g);
    return cat ? cat.tax : 0;
}

function getDiscountLabelForArticle(article) {
    const value = article.priceModification;
...
function getTaxSpecificLabelForArticle(article) {
    const value = article.taxSpecificValue;
...
function getTaxSpecificUnitForArticle(article) {
    const amount = parseFloat(article.taxSpecificAmount);
...
function renderServiceBillContent(data, sale) {
    ...
    if (articlesList.length > 0) {
        articlesList.forEach(article => {
            ...
            const taxRate = getTaxRateFromGroup(article.taxGroup);
            const specificTaxUnit = getTaxSpecificUnitForArticle(article);
            const vatOnSpecificTax = specificTaxUnit * (taxRate / 100);
            const articleTotal = price * quantity + specificTaxUnit + vatOnSpecificTax;
            totalQty += quantity;
            const taxLabel = article.taxGroup || 'null';
            const prodService = article.type ? '<span class="item-prod-service">[' + article.type + ']</span>' : '';
            const discountLabel = getDiscountLabelForArticle(article);
            const specificTaxLabel = getTaxSpecificLabelForArticle(article);
            html += '<tr class="item-name-row">';
            html += '<td colspan="2"><span class="item-name">' + (article.name || 'Article') + '<span class="item-tax-badge">' + taxLabel + '</span>' + prodService + '<small style="color:var(--success);font-weight:600;">' + discountLabel + '</small><small style="color:#b45309;font-weight:600;">' + specificTaxLabel + '</small></span></td>';
            html += '</tr>';
            html += '<tr class="item-detail-row">';
            html += '<td class="item-qty">' + quantity + ' × ' + price.toFixed(2) + ' Fc</td>';
    ...
    const printStyles = `
        <style>
            ...
            * { box-sizing: border-box; margin: 0; padding: 0; }
            ...
            /* Ticket ultra-compact : marges/espacements reduits au strict
            ...
            div[style*="border-top:1px dashed #ccc"], div[style*="border-top: 1px dashed #ccc"] {
                margin-top: 1px !important; padding-top: 1px !important; font-size: 8.5px !important; line-height: 1.1 !important;
            }
            .receipt-meta { display: flex; justify-content: space-between; font-size: 8.5px; font-weight: 600; padding: 1px 0; margin-bottom: 2px; border-bottom: 1px solid #000; }
            .receipt-items { margin-bottom: 2px; }
            .receipt-table { width: 100%; border-collapse: collapse; }
            .receipt-table th { text-align: left; padding: 1px 1px; border-bottom: 1px solid #000; font-size: 8.5px; }
            .receipt-table th:last-child { text-align: right; }
            .receipt-table .item-name-row td { padding: 0 1px 0; border-bottom: none; font-size: 9px; vertical-align: top; line-height: 1.1; }
            .receipt-table .item-detail-row td { padding: 0 1px 1px; border-bottom: 1px dashed #ccc; font-size: 8px; color: #555; font-style: italic; vertical-align: top; word-break: break-word; overflow-wrap: anywhere; line-height: 1.1; }
            ...
            .receipt-totals { margin-bottom: 1px; }
            .receipt-total-row { display: flex; justify-content: space-between; font-size: 8.5px; margin-bottom: 0; line-height: 1.15; }
            .receipt-total-row.grand-total { font-size: 12px; font-weight: 700; border-top: 2px solid #000; border-bottom: 2px solid #000; padding: 2px 0; margin-top: 1px; }
            div[style*="e8f5e9"], div[style*="4caf50"] { border-radius: 3px; padding: 2px 4px !important; margin: 2px 0 !important; font-size: 8.5px !important; }
            div[style*="e8f5e9"] div, div[style*="4caf50"] div { line-height: 1.1 !important; }
            div[style*="border:1px dashed #ccc"], div[style*="border: 1px dashed #ccc"],
            div[style*="border:2px solid #000"], div[style*="border: 2px solid #000"] {
                margin: 2px 0 !important; padding: 2px 4px !important; font-size: 8.5px !important; line-height: 1.1 !important;
            }
            .receipt-footer { text-align: center; margin-top: 2px; padding-top: 2px; border-top: 1px solid #000; font-size: 8.5px; }
            .vendeur-info { margin-bottom: 1px; }
            .qrcode-container { width: 100%; text-align: center; margin: 2px 0; overflow: visible; }
            .qrcode-container > div { display: inline-block; overflow: visible; }
            .qrcode-container svg, .qrcode-container img { display: block; margin: 0 auto; max-width: 120px; height: auto; overflow: visible; }
            .barcode { font-size: 12px; letter-spacing: 1.5px; font-weight: 700; margin: 2px 0; text-align: center; }
            .thank-you { font-style: italic; margin-top: 1px; font-size: 8.5px; }
        </style>
    `;
...
function addModalPaymentLine(type = 'ESPECES', amount = 0) {
    ...
    line.innerHTML = `
        ...
        <div>
            ...
            <input type="number" class="modal-payment-amount client-number-input" step="0.01" min="0" placeholder="0.00" value="${amount > 0 ? amount.toFixed(2) : ''}" style="width: 100%;" oninput="calculateModalPayments()">
        </div>
        <button type="button" onclick="removeModalPaymentLine(this)" style="background: #fee2e2; color: #dc2626; border: none; border-radius: 6px; width: 28px; height: 28px; display: flex; align-items: center; justify-content: center; cursor: pointer; font-size: 1rem;">×</button>
    `;
...
function updateModalPaymentRemoveButtons() {
    const lines = document.querySelectorAll('.modal-payment-line');
    lines.forEach(line => {
        const btn = line.querySelector('button[onclick^="removeModalPaymentLine"]');
...
function calculateModalPayments() {
    ...
    const cartTotal = posCart.currentTotals?.total || 0;
...
function highlightModalField(fieldId) {
...
function confirmInvoiceInfo() {
    ...
    if (clientNumber && !PHONE_NUMBER_REGEX.test(clientNumber)) {
        ...
        highlightModalField('modal-client-number');
        return;
    }

    // Extraire le code client depuis le texte de l'option sélectionnée (ex: "PP - Particulier")
    const clientTypeSelect = document.getElementById('modal-client-type');
    const clientTypeText = clientTypeSelect?.options?.[clientTypeSelect.selectedIndex]?.textContent || '';
    ...
    const payments = getModalPayments();
    const cartTotal = posCart.currentTotals?.total || 0;
    const paymentsTotal = payments.reduce((sum, p) => sum + p.amount, 0);
    if (payments.length === 0 || paymentsTotal < cartTotal - 0.001) {
        ...
        if (firstAmount) {
            highlightModalField(firstAmount.id || 'modal-payments-list');
        }
        return;
    }
